...
class TableTest {
    ...
    void removeCard() {
        ...
        int cardToRemove = table.slotToCard[2];
        table.removeCard(2);
        // calculate the expected score for later
        Object expectedSlotToCard = null;

        Object expectedCardToSlot = null;
        // check that the slot was emptied correctly
        assertEquals(expectedSlotToCard, table.slotToCard[2]);

        // check that the card was removed correctly
        assertEquals(expectedCardToSlot, table.cardToSlot[cardToRemove]);
    ...
    void placeCard() {
        ...
        int cardToPlace = table.cardToSlot[2];
        table.placeCard(2, 0);

        // check that the card is placed where it should correctly
        //the card in slot 0 is 2
        assertEquals(cardToPlace, table.slotToCard[0]);
